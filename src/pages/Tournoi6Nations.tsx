...
import { Calendar, MapPin, Train, Trophy, Clock, Car, Ticket } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto text-center">
              <div className="flex items-center justify-center gap-4 mb-6">
                ...
                <h2 className="font-display text-3xl md:text-4xl text-foreground">{t('rugby.unmissable')}</h2>
                <img src={drapeauIrlande} alt="Irlande" className="w-8 h-6 object-cover rounded-sm shadow" />
              </div>
              <p className="text-muted-foreground text-lg leading-relaxed mb-6" dangerouslySetInnerHTML={{ __html: t('rugby.tournamentDesc') }} />
              <p className="text-muted-foreground text-lg leading-relaxed">{t('rugby.prestigeDesc')}</p>
              <div className="mt-8 p-6 bg-white dark:bg-card rounded-xl shadow-lg border border-border">
                <p className="text-foreground font-medium text-lg">🏨 {t('rugby.welcome')}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="py-16 bg-gradient-to-br from-blue-100 via-white to-green-100 dark:from-blue-950/30 dark:via-card dark:to-green-950/30">
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto">
              <div className="text-center mb-10">
                <h2 className="font-display text-3xl md:text-4xl text-foreground mb-2">{t('rugby.exclusiveOffers')}</h2>
                <p className="text-muted-foreground">{t('rugby.offersSubtitle')}</p>
              </div>
              <div className="grid md:grid-cols-2 gap-8">
                <div className="bg-white dark:bg-card rounded-2xl p-8 shadow-xl border-2 border-blue-200 dark:border-blue-800/50 hover:scale-105 transition-transform duration-300">
                  <div className="w-16 h-16 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center mb-4"><Car className="w-8 h-8 text-blue-600" /></div>
                  <h3 className="font-display text-2xl text-foreground mb-2">{t('rugby.parking')}</h3>
                  <p className="text-muted-foreground mb-4">{t('rugby.parkingDesc')}</p>
                  <div className="inline-block bg-blue-600 text-white font-bold text-xl px-4 py-2 rounded-full">{t('rugby.parkingPrice')}</div>
                </div>
                <div className="bg-white dark:bg-card rounded-2xl p-8 shadow-xl border-2 border-green-200 dark:border-green-800/50 hover:scale-105 transition-transform duration-300">
                  <div className="w-16 h-16 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center mb-4"><Ticket className="w-8 h-8 text-green-600" /></div>
                  <h3 className="font-display text-2xl text-foreground mb-2">{t('rugby.taxi')}</h3>
                  <p className="text-muted-foreground mb-4">{t('rugby.taxiDesc')}</p>
                  <div className="inline-block bg-green-600 text-white font-bold text-xl px-4 py-2 rounded-full">{t('rugby.taxiDiscount')}</div>
                </div>
              </div>
              <p className="text-center text-sm text-muted-foreground mt-6 italic">{t('rugby.offersNote')}</p>
            </div>
          </div>
        </section>
